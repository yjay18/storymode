...
from domain.models.party_state import CompanionRuntimeState, PartyState
...
def test_resolve_success_with_roll(base_state: RuntimeState) -> None:
    ...
    outcomes = CheckOutcomes(
        natural_1=[], low=[], strong=[], natural_20=[],
        standard=[EffectDefinition(effect_id="eff-heal", kind=EffectKind.HEAL, magnitude=2)]
    )
    
    check = PendingCheck(
        check_id="chk-1", source_command_id="cmd-1", source_revision=1,
        original_input="Jump", resolved_operation="investigate", actor_id="player-1", target_ids=["player-1"],
        semantic_difficulty="standard", base_dc=12, difficulty_adjustment=0, final_dc=12, stakes="Fall",
...
def test_resolve_with_luck(base_state: RuntimeState) -> None:
    ...
    new_state, roll, band, effects = resolver.resolve_check(state, use_luck=True)
