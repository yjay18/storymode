...
def test_save_writer_write_state(temp_campaign_root: Path, mock_state: RuntimeState, mock_meta: SaveMeta) -> None:
    writer = SaveWriter(temp_campaign_root)
    writer.write_state(mock_state, mock_meta, None)
    
    save_dir = temp_campaign_root / "campaigns" / mock_state.campaign_id / "saves" / mock_state.save_id
...
def test_save_writer_atomic_replace_failure(temp_campaign_root: Path, mock_state: RuntimeState, mock_meta: SaveMeta) -> None:
    writer = SaveWriter(temp_campaign_root)
    save_dir = temp_campaign_root / "campaigns" / mock_state.campaign_id / "saves" / mock_state.save_id
    ...
    with patch("os.replace", side_effect=OSError("Disk full")):
        with pytest.raises(SaveError, match="Failed to write"):
            writer.write_state(mock_state, mock_meta, None)
...
def test_save_writer_flush_prepared_rows(temp_campaign_root: Path, mock_state: RuntimeState) -> None:
    writer = SaveWriter(temp_campaign_root)
