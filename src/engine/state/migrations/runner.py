...
class MigrationRunner:
    ...
    def run_migrations(self, state: dict[str, Any], target_version: int = 1) -> dict[str, Any]:
        ...
        if current_version > target_version:
            raise ValueError(
                f"State schema version {current_version} is newer than target {target_version}"
            )
