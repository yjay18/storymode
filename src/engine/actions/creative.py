    ...
    def validate(
        ...
        if target.type != "object":
            raise OperationValidationError(
                f"Creative interactions require an object target, got {target.type}"
            )

        area_obj = area_objects.get(target.id)
        if not area_obj:
            raise OperationValidationError(
                f"Target object {target.id} not found in area definition"
            )
        ...
        # Map capability mentions to real items/skills to verify requirements.
        ...
        for comp_id in state.party.active_companion_ids:
            ...
            if comp:
                # Add companion ID as an available capability
                available_caps.add(comp_id.lower())

        # Check requirements against mentioned and possessed capabilities
