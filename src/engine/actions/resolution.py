...
class CheckResolver:
    ...
    def resolve_check(self, state: RuntimeState, use_luck: bool) -> tuple[RuntimeState, int, str, list[EffectDefinition]]:
        ...
        if use_luck:
            ...
            new_player = new_player.model_copy(update={"luck_current": state.player.luck_current - 1})
            roll = 20
        else:
        ...
        for effect in effects:
            if effect.kind == EffectKind.DAMAGE:
                ...
                if target_ids:
                    t_id = target_ids[0]
                    if t_id == new_state.player.id:
                        new_hp = min(new_state.player.hp.maximum, new_state.player.hp.current + effect.magnitude)
                        new_hp_val = new_state.player.hp.model_copy(update={"current": new_hp})
                        new_player = new_state.player.model_copy(update={"hp": new_hp_val})
                        new_state = new_state.model_copy(update={"player": new_player})
                    else:
