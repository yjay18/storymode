...
def validate_directory(campaign_dir: Path) -> int:
    ...
    _pack, diagnostics = load_campaign(campaign_dir)

    if not diagnostics:
        print("Validation successful: 0 diagnostics.")  # noqa: T201
        return 0

    print(f"Validation failed: {len(diagnostics)} diagnostics found.")  # noqa: T201
    for d in sorted(diagnostics):
        print(f"[{d.file}] {d.code} at {d.json_pointer}: {d.message}")  # noqa: T201
...
def main() -> None:
    ...
    if not args.directory.is_dir():
        print(f"Error: Directory '{args.directory}' does not exist.")  # noqa: T201
        sys.exit(1)
