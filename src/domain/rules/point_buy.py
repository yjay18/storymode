...
def validate_point_buy(stats: dict[StatName, int], definition: PointBuyDefinition) -> PointBuyResult:
...
def apply_background_bonus(
    ...
    if set(stats.keys()) != set(StatName):
        return BackgroundApplyResult(
            stats=None,
            errors=["Input stats must contain exactly the six core statistics"]
        )
