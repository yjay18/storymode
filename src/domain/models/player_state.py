...
class PlayerState(FrozenModel):
    ...
    def check_invariants(self) -> "PlayerState":
        ...
        if self.luck_current > self.luck_capacity:
            msg = (
                f"luck_current ({self.luck_current}) cannot exceed "
                f"luck_capacity ({self.luck_capacity})"
            )
        ...
        for skill_id, rank in self.non_combat_skill_ranks.items():
            if not (0 <= rank <= 5):
                raise ValueError(
                    f"non_combat_skill_ranks for {skill_id} must be between 0 and 5, got {rank}"
                )
        ...
        if len(self.combat_loadout) > 4:
            raise ValueError(
                f"combat_loadout cannot exceed 4 items, got {len(self.combat_loadout)}"
            )
        ...
        # Equipment must be a subset of inventory (all equipped IDs must appear in inventory)
