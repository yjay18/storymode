...
class ExplorationUseCases:
    ...
    def submit_action(
        ...
        state, meta = self.save_reader.read_save(campaign_id, save_id)
        ...
        if decide_check_necessity(proposal):
            ...
            # Real implementation would calculate DC and fetch outcomes based on difficulty
            empty_outcomes = CheckOutcomes(natural_1=[], low=[], standard=[], strong=[], natural_20=[])
            ...
            # Do not increment revision yet, wait for resolution or just save
            
        # 4. Save
        # Increment revision if state changed
