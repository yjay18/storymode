...
class SaveReader:
    ...
    def load_save(
        ...
        if state.campaign_id != campaign_id:
            raise CampaignMismatchError(f"State campaign_id {state.campaign_id} != expected {campaign_id}")
        if meta is not None and meta.campaign_id != campaign_id:
            raise CampaignMismatchError(f"Meta campaign_id {meta.campaign_id} != expected {campaign_id}")
            
        if expected_fingerprint is not None:
            if state.campaign_fingerprint != expected_fingerprint:
    ...
    def _read_json_model(self, path: Path, model_cls: type[Any]) -> Any:
        ...
        try:
            ...
            raise CorruptSaveError(f"Invalid JSON in {path}: {e}")
        except ValidationError as e:
            raise CorruptSaveError(f"Validation failed for {path}: {e}")
    ...
    def _read_jsonl(
        ...
        try:
            ...
            raise CorruptSaveError(f"Invalid JSON line in {path}: {e}")
        except ValidationError as e:
            raise CorruptSaveError(f"Validation failed for {path}: {e}")
